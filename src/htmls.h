...
var o=await fetch('/api/lang').then(r=>r.json());window._t=T[o.lang]!==undefined?T[o.lang]:T[0];document.title=window._t.title;document.documentElement.lang=['en','de','es','fr'][o.lang]||'en';document.querySelectorAll('[data-i18n]').forEach(function(el){var k=el.getAttribute('data-i18n');if(el.id==='backupBtnText'||el.id==='restoreBtnText')el.textContent=el.id==='backupBtnText'?window._t.downloadBtn:window._t.restoreBtn;else if(k&&window._t[k])el.textContent=window._t[k];});
