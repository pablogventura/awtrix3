...
static const char settings_html[] PROGMEM = R"EOF(
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Display Settings</title><link href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet"></head><body class="bg-light py-4"><div class="container"><h2 class="mb-4">Display Settings</h2><p class="text-muted">Configure brightness, transitions, apps and more. Changes apply immediately.</p><form id="f"><div class="form-group"><label>Brightness (0-255)</label><input type="number" class="form-control" id="BRI" min="0" max="255"></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="ABRI"><label class="form-check-label" for="ABRI">Auto brightness</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="ATRANS" checked><label class="form-check-label" for="ATRANS">Auto switch apps</label></div><div class="form-group"><label>Time per app (s)</label><input type="number" class="form-control" id="ATIME" min="1" max="120"></div><div class="form-group"><label>Transition speed (ms)</label><input type="number" class="form-control" id="TSPEED" min="100" max="3000"></div><div class="form-group"><label>Transition effect (0-10)</label><input type="number" class="form-control" id="TEFF" min="0" max="10"></div><div class="form-group"><label>Time format</label><input type="text" class="form-control" id="TFORMAT" placeholder="%H %M"></div><div class="form-group"><label>Date format</label><input type="text" class="form-control" id="DFORMAT" placeholder="%d.%m.%y"></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="SOM"><label class="form-check-label" for="SOM">Week starts Monday</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="CEL"><label class="form-check-label" for="CEL">Celsius</label></div><div class="form-group"><label>Menu language (0=EN 1=DE 2=ES 3=FR)</label><input type="number" class="form-control" id="MLANG" min="0" max="3"></div><div class="form-group"><label>Volume (0-30)</label><input type="number" class="form-control" id="VOL" min="0" max="30"></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="SOUND"><label class="form-check-label" for="SOUND">Sound on</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="TIM"><label class="form-check-label" for="TIM">Show Time app</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="DAT"><label class="form-check-label" for="DAT">Show Date app</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="TEMP"><label class="form-check-label" for="TEMP">Show Temperature app</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="HUM"><label class="form-check-label" for="HUM">Show Humidity app</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="BAT"><label class="form-check-label" for="BAT">Show Battery app</label></div><div class="form-check mb-2"><input type="checkbox" class="form-check-input" id="WD"><label class="form-check-label" for="WD">Show weekday bar</label></div><button type="submit" class="btn btn-primary">Save</button></form><p class="mt-4"><a href="/backup">Back to Backup</a></p><p id="msg" class="text-success font-weight-bold" style="display:none">Saved.</p></div><script>
document.addEventListener('DOMContentLoaded',function(){
fetch('/api/settings').then(function(r){return r.json();}).then(function(d){
if(d.BRI!=undefined)document.getElementById('BRI').value=d.BRI;
if(d.ABRI!=undefined)document.getElementById('ABRI').checked=d.ABRI;
if(d.ATRANS!=undefined)document.getElementById('ATRANS').checked=d.ATRANS;
if(d.ATIME!=undefined)document.getElementById('ATIME').value=d.ATIME;
if(d.TSPEED!=undefined)document.getElementById('TSPEED').value=d.TSPEED;
if(d.TEFF!=undefined)document.getElementById('TEFF').value=d.TEFF;
if(d.TFORMAT!=undefined)document.getElementById('TFORMAT').value=d.TFORMAT;
if(d.DFORMAT!=undefined)document.getElementById('DFORMAT').value=d.DFORMAT;
if(d.SOM!=undefined)document.getElementById('SOM').checked=d.SOM;
if(d.CEL!=undefined)document.getElementById('CEL').checked=d.CEL;
if(d.MLANG!=undefined)document.getElementById('MLANG').value=d.MLANG;
if(d.VOL!=undefined)document.getElementById('VOL').value=d.VOL;
if(d.SOUND!=undefined)document.getElementById('SOUND').checked=d.SOUND;
if(d.TIM!=undefined)document.getElementById('TIM').checked=d.TIM;
if(d.DAT!=undefined)document.getElementById('DAT').checked=d.DAT;
if(d.TEMP!=undefined)document.getElementById('TEMP').checked=d.TEMP;
if(d.HUM!=undefined)document.getElementById('HUM').checked=d.HUM;
if(d.BAT!=undefined)document.getElementById('BAT').checked=d.BAT;
if(d.WD!=undefined)document.getElementById('WD').checked=d.WD;
}).catch(function(){document.getElementById('msg').textContent='Failed to load settings'; document.getElementById('msg').style.display='block';});
document.getElementById('f').onsubmit=function(e){e.preventDefault();var o={};var ids=['BRI','ATIME','TSPEED','TEFF','TFORMAT','DFORMAT','MLANG','VOL'];ids.forEach(function(id){var el=document.getElementById(id);if(el&&el.value!=='')o[id]=el.tagName==='INPUT'&&el.type==='number'?parseInt(el.value,10):el.value;});['ABRI','ATRANS','SOM','CEL','SOUND','TIM','DAT','TEMP','HUM','BAT','WD'].forEach(function(id){var el=document.getElementById(id);if(el)o[id]=el.checked;});fetch('/api/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(o)}).then(function(r){if(r.ok){var m=document.getElementById('msg');m.textContent='Saved.';m.style.display='block';setTimeout(function(){m.style.display='none';},2000);}});return false;};
});
</script></body></html>
)EOF";