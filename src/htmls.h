...
document.addEventListener('DOMContentLoaded',async function(){
var o=await fetch('/api/lang').then(r=>r.json());window._t=T[o.lang]!==undefined?T[o.lang]:T[0];document.title=window._t.title;document.getElementById('heading').textContent=window._t.heading;document.getElementById('subtitle').textContent=window._t.subtitle;document.getElementById('backLink').textContent=window._t.back;document.documentElement.lang=['en','de','es','fr'][o.lang]||'en';
